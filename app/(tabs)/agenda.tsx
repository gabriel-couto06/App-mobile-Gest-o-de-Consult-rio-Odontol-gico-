...
export default function Agenda() {
  ...
  const { consultas, confirmarConsulta, cancelarConsulta, enviarConfirmacaoWhatsApp, marcarComoConfirmada } = useAgenda();
...
function renderHorario(horario: string) {
  const consulta = consultasDoDia.find(
    (c) => c.hora === horario
  );
    ...
    >  
      ...
      <View style={styles.cardBody}>
        ...
        <Text style={styles.horario}>
          ⏰ <Text style={{fontWeight: 600 }}>{consulta.hora} - {somar30Min(consulta.hora)}</Text>
          {consulta.tipo === 'manutencao' &&  <Text style={{fontWeight: 700, color: '#9b59b6'}}>  (Manutenção)</Text>}
        </Text>

        {consulta.obs ? (
          <Text style={styles.obs}>{consulta.obs}</Text>
        ) : null}
        ...
        {consulta.confirmacaoWhatsapp === 'nao_enviado' || !consulta.confirmacaoWhatsapp ? (
  <TouchableOpacity
  ...
  >
    <View style={[styles.badge, {minWidth: 13, height: 13, right: -3, top: -3, backgroundColor: '#c41400',}]}></View>
...
{!painelAberto && !painelConfigAberto &&(
  ...
  <View style={{ alignItems: 'center' }}>
    ...
    width: 145, height: 8, marginBottom: 18 }}></Text>
...
<>
        ...
        {/* DIAS */}
        <ScrollView ref={diasScrollRef} horizontal showsHorizontalScrollIndicator={false}>
          {dias.map((d) => {
            const dataDia = new Date(anoAtual, mesAtual, d);

            const diaSemanaNumero = dataDia.getDay();
            const fimDeSemana = diaSemanaNumero === 0 || diaSemanaNumero === 6;
...
{modoVisualizacao === 'semana' && (
  ...
  {consulta ? (
    ...
    >
      {consulta.confirmacaoWhatsapp === 'pendente' && (
    <View style={[styles.badgeMini, { backgroundColor: '#cbb400' }]} />
  )}

  {consulta.confirmacaoWhatsapp === 'confirmado' && (
    <View style={[styles.badgeMini, { backgroundColor: '#078b37' }]} />
  )}
      <Text style={styles.nomeSemana}>
        {consulta.paciente.split(' ')[0]}
      </Text>
      <Text style={styles.horaSemana}>
        (Visualizar)
      </Text>
    </TouchableOpacity>
  ) : (
...
{modalConsultaAberto && consultaSelecionada && (
  <Modal
    transparent
    animationType="fade"
    visible={modalConsultaAberto}
  >
    <View style={styles.modalContainer}>
  <View style={[styles.modalCard, {paddingBottom: 2, paddingTop: 13}]}>
    ...
    <Text style={styles.horario}>
      ⏰ {consultaSelecionada.hora} - {somar30Min(consultaSelecionada.hora)}
    </Text>
    ...
    <View style={styles.actionsRow}>
      ...
      >
       ...
       {/* WHATSAPP */}
    {consultaSelecionada.confirmacaoWhatsapp === 'nao_enviado' || !consultaSelecionada.confirmacaoWhatsapp ? (
      <TouchableOpacity
        style={styles.btnWhatsapp}
        onPress={() => {
