...
export default function PacienteDetalhe() {
  ...
  const { pacientes, limparHistorico, salvarDente, removerDente, toggleTratado, salvarPlanejamentoOrto, atualizarPaciente, adicionarImagem,
  removerImagem,
  adicionarArquivo,
  removerArquivo } = usePaciente();
...
useEffect(() => {
  ...
  const [aba, setAba] = useState<
    'Resumo' | 'Histórico' | 'Odontograma' | 'Ficha' | 'Financeiro' >('Resumo');
...
const [formaPagamento, setFormaPagamento] = useState<'pix' | 'credito' | 'debito' | 'dinheiro'>('pix');
...
function getIconByFileName(nome: string) {
  ...
  switch (ext) {
    case 'pdf':
      return <MaterialIcons name="picture-as-pdf" size={26} color="#E53935" />;
    ...
    case 'xls':
    case 'xlsx':
      return <FontAwesome5 name="file-excel" size={22} color="#2E7D32" style={{ marginLeft: 4 }}/>;
...
const dentesPendentes = paciente.odontograma.filter(
  (d) => !d.tratado
);

const dentesTratados = paciente.odontograma.filter(
  (d) => d.tratado
);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#EEF4FB' }}>
      ...
      <View style={styles.tabs}>
        {['Resumo', 'Histórico', 'Odontograma', 'Ficha', 'Financeiro'].map((t) => (
...
{/* ============================== CONTEÚDO ===================================== */}
      ...
      <ScrollView style={{ padding: 16 }}>
        ...
        {aba === 'Histórico' && (
          ...
          <>
            {/* PRÓXIMA CONSULTA */}
            <Text style={[styles.sectionTitle, {marginTop: 4}]}>Próxima consulta</Text>
...
<View style={styles.cardHistoricoProximo}>
  ...
  <Text style={{fontSize: 16, color: '#3b4262', fontWeight: '500'}}>{proximaConsulta.obs}</Text>
  <Text style={{marginBottom: 2, marginTop: 1}}>
      📅 <Text style={{color: '#8a8b9d', fontWeight: '500'}}>{new Date(proximaConsulta.data).toLocaleDateString()}</Text>   ⏰ <Text style={{fontWeight: 'bold', color: '#777'}}>{proximaConsulta.hora}</Text>
    </Text>
        ...
        <Text style={{borderBottomWidth: 0.5, borderColor: '#777' }}></Text>

{/* ===== CONSULTAS ANTERIORES ===== */}
<Text style={[styles.sectionTitle, {color: '#0097A7', marginTop: 18, fontSize: 18 }]}>Consultas anteriores</Text>

{historicoConsultas.length === 0 ? (
  <Text style={styles.empty}>Nenhuma consulta realizada ainda</Text>
) : historicoConsultas.length <= 2 ? (
  historicoConsultas.map((h) => (
    <View key={h.id} style={[styles.card, styles.cardHistorico]}>
      <Text>📅 <Text style={{color: '#888', fontWeight: '500'}}>
  ...
  <ScrollView style={{ maxHeight: 180, borderWidth: 1.5, borderRadius: 3, borderColor: "#dbdbdb" }}>
    {historicoConsultas.map((h) => (
      <View key={h.id} style={[styles.card, styles.cardHistorico]}>
        <Text>📅 <Text style={{color: '#888', fontWeight: '500'}}>
          {new Date(h.data).toLocaleDateString()}
...
{historicoManutencoes.length > 0 && (
  <>
    <Text style={[styles.sectionTitle, {color: '#0097A7', marginTop: 10 , fontSize: 18 }]}>
      Manutenções anteriores
    </Text>

    {historicoManutencoes.length <= 2 ? (
      historicoManutencoes.map((h) => (
        <View key={h.id} style={[styles.card, styles.cardManutencao]}>
          <Text>📅 <Text style={{color: '#888', fontWeight: '500'}}>
            {new Date(h.data).toLocaleDateString()}
          </Text></Text>
          {h.obs ? (
            <Text>🔧 {h.obs.replace('Manutenção ortodôntica - ', '')}</Text>
          ) : null}
        </View>
      ))
    ) : (
...
{aba === 'Odontograma' && (
  <>
    <Text style={[styles.sectionTitle, {marginTop: 4, textAlign: 'center' }]}>Mapa Dentário</Text>

   {/* MAPA ODONTOGRAMA */}
<View style={styles.mapaImagemContainer}
...
<Image
  ...
  {/* OVERLAY DOS DENTES */}
  {FDI_DENTES.map((d) => {
  const registro = paciente.odontograma.find((r) => r.numero === d.numero);
  const isSelecionado = denteSelecionado?.numero === d.numero;
  const isTratado = registro?.tratado;

  let borderColor = 'transparent';

  if (isTratado) borderColor = '#2ecc71';       // verde
  else if (registro) borderColor = '#f1c40f';   // amarelo
...
<View style={{ flexDirection: 'row', gap: 15 }}>
 <TouchableOpacity
        ...
        >
          <Text style={{ color: '#777777', marginTop: 8 }}>Voltar</Text>
        </TouchableOpacity>
    ...
    {/* LISTA DE REGISTROS */}
<Text style={[styles.sectionTitle, {marginTop: 9 , marginBottom: 3 ,fontSize: 18 }]}>Registros</Text>

{paciente.odontograma.length === 0 ? (
  <Text style={styles.empty}>Sem registros atualmente</Text>
) : (
  <ScrollView style={{ borderBottomWidth: 1.5, borderRadius: 3, borderColor: "#dbdbdb" }}
    horizontal
  ...
  style={[
    ...
    mostrarOrto && { backgroundColor: '#777777', borderColor: '#333' },
  ]}
  onPress={() => {
...
>
  <Text
  >
    {mostrarOrto ? <Text style={{ color: '#ebebeb', fontSize: 16, fontWeight: '700' }}>- Minimizar</Text> : <Text style={{ color: '#ebebeb', fontSize: 15.5, fontWeight: '700' }}>+ Abrir Planejamento Ortodôntico</Text> }
  </Text>
</TouchableOpacity>
...
{mostrarOrto && (
  <>
    {/* ===== RESUMO ORTODÔNTICO ===== */}
    <Text style={[styles.sectionTitle, {textAlign: 'center', marginBottom: 0.2, marginTop: 20 }]}>Ortodontia</Text>
     <Text style={{ backgroundColor: '#035087',
          borderBottomLeftRadius: 14,
          borderBottomRightRadius: 14,
          width: 140, height: 4, marginBottom: 15, alignSelf: 'center' }}></Text>

    <View style={styles.card}>
      <View style={styles.cardResumoOrto}>
        <Text style={{ fontWeight: '700', marginBottom: 10, color: '#383838', fontSize: 16 }}>
          🦷 Resumo Ortodôntico
        </Text>
          ...
          <View style={styles.boxOrto}>
            ...
            {editandoOrto ? (
              <TextInput
                ...
                onChangeText={(t) => setOrtoDataInicio(formatarData(t))}
    ...
    {/* ===== PRÓXIMA MANUTENÇÃO ===== */}
    <Text style={[styles.sectionTitle, {fontSize: 18, marginTop: 10 }]}>Próxima manutenção</Text>
    <View style={[styles.card, {marginBottom: 10 }]}>
      {proximaManutencao ? (
        <>
        <View style={styles.cardResumoOrto}>
          <Text style={styles.cardTitle}>Manutenção agendada</Text>
          <Text>📅 {new Date(proximaManutencao.data).toLocaleDateString()}  -  ⏰ {proximaManutencao.hora}</Text>
          <Text>📝 {proximaManutencao.obs}</Text>
          </View>
        </>
      ) : (
        <Text style={{ color: '#777' }}>Nenhuma manutenção agendada</Text>
      )}

      <TouchableOpacity
        style={[
          styles.btnSalvar,
          { marginTop: 12, backgroundColor: '#9b59b6', alignSelf: 'center' },
        ]}
        onPress={() => setMostrarManutencaoForm(true)}
      >
        <Text style={{ color: '#fff', fontWeight: '700', fontSize: 14 }}>+ Agendar manutenção</Text>
      </TouchableOpacity>
    </View>

    {/* Elásticos e Acessórios */}
    <Text style={[styles.sectionTitle, {fontSize: 18, marginTop: 10 }]}>Elásticos e Acessórios</Text>

<View style={{ flexDirection: 'row', gap: 10, marginBottom: 100 }}>

  {/* CARDS DE ACESSÓRIOS */}
  {acessorios.map((item) => (
    <View key={item.id} style={[styles.card, styles.cardAcessorio, { marginBottom: 10 }]}>
      ...
      {/* INSTRUÇÃO */}
      <Text style={[styles.labelOrto, { marginTop: 6 }]}>📋 Instrução de uso</Text>
      {item.editando ? (
        <TextInput
        ...
        <Text style={styles.valorOrto}>{item.instrucao || '—'}</Text>
      )}

      {/* ADESÃO */}
      <Text style={[styles.labelOrto, { marginTop: 6 }]}>🙂 Adesão</Text>
          ...
          {['boa', 'irregular'].map((op) => (
            <TouchableOpacity
              ...
              onPress={() =>
                setAcessorios((prev) =>
                  prev.map((a) =>
                    a.id === item.id ? { ...a, adesao: op as any } : a
                  )
                )
              }
            >
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          setAcessorios((prev) =>
            prev.map((a) =>
              a.id === item.id ? { ...a, editando: !a.editando } : a
            )
          )
        }
      >
...
>
  <Text style={{ color: '#fff', fontWeight: '700', fontSize: 11 }}>🗑️ Excluir</Text>
</TouchableOpacity>
    </View>
    </View>
  ))}
  ...
  {acessorios.length < 2 && (
    <TouchableOpacity
      style={[styles.card, styles.cardNovoAcessorio, { marginBottom: 10 }]}
      onPress={() =>
        setAcessorios((prev) => [
          ...prev,
          {
...
{mostrarManutencaoForm && (
  <View style={styles.modalOverlay}>
    <View style={styles.modalCard}>
      ...
      <TouchableOpacity onPress={() => setMostrarManutencaoForm(false)}>
...
<View>
<Text style={[styles.sectionTitle, {marginTop: 4 }]}>Ficha do Paciente</Text>
...
{modoEdicao ? (
  <>
    <View style={{ flexDirection: 'row', gap: 8, marginTop: 6 }}>
      {['sim', 'nao'].map((op) => (
        <TouchableOpacity
          ...
          style={[
            styles.btnAdesao,
            dadosEditados.convenio === op && styles.btnAdesaoAtivo,
          ]}
          onPress={() =>
            setDadosEditados(prev => ({
              ...prev,
              convenio: op as 'sim' | 'nao',
              nomeConvenio: op === 'nao' ? '' : prev.nomeConvenio
            }))
          }
        >
          <Text
            style={{
              color: dadosEditados.convenio === op ? '#fff' : '#555',
              fontWeight: '600',
            }}
          >
            {op === 'sim' ? 'sim' : 'não'}
    ...
    {dadosEditados.convenio === 'sim' && (
      <TextInput
        ...
        onChangeText={(t) =>
          setDadosEditados(prev => ({ ...prev, nomeConvenio: t }))
        }
  ...
  <View style={styles.enderecoContainer}>
    <Text style={[styles.sectionTitle, {fontSize: 17 }]}>Endereço</Text>
...
{modoEdicao ? (
  ...
  <View style={{flexDirection: 'row', gap: 20 }}>
    <TextInput
      ...
      style={[styles.input, {width:100, marginTop: 0, marginBottom: 8 }]}
    />
    ...
    <TextInput
      ...
      onChangeText={(t) =>
        setDadosEditados(prev => ({ ...prev, complemento: t }))
      }
      style={[styles.input, {width: 100, marginTop: 0, marginBottom: 8 }]}
    />
    </View>
...
<Text style={[styles.sectionTitle, {marginTop: 27 }]}>Imagens e Arquivos</Text>
<View style={[styles.card, {marginBottom: 100 }]}>

  <Text style={[styles.subTitle,{marginTop: 2}]}>📷 Imagens Clínicas</Text>

  <TouchableOpacity style={styles.botaoAdicionar} onPress={adicionarImagemHandler}>
  ...
  <View key={index} style={{ position: 'relative' }}>
    <TouchableOpacity onPress={() => setImagemSelecionada(img)}>
      <Image source={{ uri: img }} style={styles.imagemGrid} />
    </TouchableOpacity>
  ...
  <TouchableOpacity style={styles.botaoAdicionar} onPress={adicionarArquivoHandler}>
...
{aba === 'Resumo' && (
  <View>
    ...
    <View style={[styles.resumoGrid, {marginBottom: 2}]}>
      ...
      <View style={styles.cardResumo}>
        <Text style={styles.resumoTitulo}>Idade: <Text style={{fontWeight: 900 }}>{paciente.idade ? `${paciente.idade} anos` : '—'}</Text></Text>

    <Text style={{ borderWidth: 0.3, height: 0.3, borderColor: '#cacaca', marginTop: 15 }}></Text>
  ...
  <Text style={styles.resumoValor}>
   <Text style={{fontSize: 11 }}>{`📞`}</Text> {paciente.telefone || '—'}
  </Text>
</TouchableOpacity>
...
<Text style={{ borderWidth: 0.3, height: 0.3, borderColor: '#cacaca', marginTop: 5, marginBottom: 6 }}></Text>
    ...
    <Text style={{borderBottomWidth: 0.5, borderColor: '#b4b4b4', marginTop: 3 }}></Text>

 <Text style={[styles.sectionTitle, {marginTop: 12, textAlign: 'center', fontSize: 18}]}>Próxima consulta</Text>

            {proximaConsulta ? (
  <View style={[styles.card, styles.cardAgendada, {marginBottom: 4}]}>
  ...
  <Text style={{marginBottom: 2, marginTop: 1}}>
    ...
    <Text style={[styles.empty, {textAlign: 'center'}]}>Nenhuma consulta marcada 😴</Text>
  )}

<Text style={{borderBottomWidth: 0.5, borderColor: '#b4b4b4'}}></Text>

<Text style={[styles.sectionTitle, {marginTop: 14, fontSize: 18}]}>Odontograma</Text>
<View style={styles.mapaImagemContainer}
 onLayout={(e) => setMapaWidth(e.nativeEvent.layout.width)}>
<Image
    source={require('../../assets/images/Odontograma.jpeg')}
  ...
  <View style={[styles.card, {flexDirection: 'row', justifyContent: 'space-evenly', width: '70%', alignSelf: 'center', marginTop: 1 }]}>
  ...
  <>
    {/* TOTAL */}
    <View style={styles.card}>
      <Text style={[styles.cardTitle, {fontSize: 17 }]}>💰 Total devido</Text>
      <Text style={{ fontSize: 20, fontWeight: '700', marginTop: 3, color: "#b40000" }}>
        R$ {totalDevido.toFixed(2)}
      </Text>

      <TouchableOpacity
        style={[styles.btnSalvar, {backgroundColor: '#0a3d62', marginTop: 10 }]}
        onPress={() => setModalCobranca(true)}
      >
        <Text style={{ color: '#fff', fontWeight: '500' }}>+ Adicionar cobrança</Text>
      </TouchableOpacity>
    </View>

    {/* PENDENTES */}
    <Text style={[styles.sectionTitle, {marginTop: 12}]}>Pagamentos pendentes</Text>
...
{cobrancasPendentes.length === 0 ? (
  ...
  cobrancasPendentes.map((c) => {
    ...
    return (
      <View key={c.id} style={[styles.card, {paddingTop: 7, paddingBottom: 12, paddingHorizontal: 12, width: 290, marginRight: 10 }]}>
  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
    
    {/* ESQUERDA */}
    <View style={{ flex: 1 }}>
      {c.descricao ? <Text style={{fontSize: 16, color: '#3b4262', fontWeight: '500'}}>{c.descricao}</Text> : null}
      <Text style={{color: '#8a8b9d', fontWeight: '500'}}>{c.data}</Text>
    </View>
  ...
  <Text style={{color: '#007C91', fontWeight: '600' }}>
    💳 Parcela {c.parcelas.atual + 1} de {c.parcelas.total}
  </Text>
)}
    <Text style={{ fontWeight: '700', color: '#b40000', alignSelf: 'flex-end' }}>
...
{c.parcelas && c.parcelas.atual < c.parcelas.total ? (
  <TouchableOpacity
    style={[styles.btnSalvar, { paddingVertical: 8,
  ...
  >
    ...
    {/* HISTÓRICO */}
    <Text style={[styles.sectionTitle, {marginTop: 12}]}>Histórico de pagamentos</Text>
...
return (
  ...
  <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
  <Text style={{color: "#654A24", fontSize: 15, fontWeight: '600'}}>Pago</Text>
  ...
  <View style={{borderBottomWidth: 0.2, marginTop: 5 ,marginBottom: 6, borderColor: '#bdbdbd'}}></View>

  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>

    <View style={{ flex: 1 }}>
      {cobranca?.descricao ? (
        <Text  style={{fontSize: 16, color: '#3b4262', fontWeight: '500'}}>{cobranca.descricao}</Text>
      ) : null}
      <Text style={{color: '#8a8b9d', fontWeight: '500'}}>{p.data}</Text>
    </View>

    <View>
      <Text  style={{fontSize: 16, color: '#3b4262', fontWeight: '500'}}>R$ {p.valor.toFixed(2)}</Text>
      <Text style={{color: '#8a8b9d', fontWeight: '500', alignSelf: 'flex-end'}}>💳 {p.forma}</Text>
    </View>
  ...
  <ScrollView style={{ maxHeight: 215, borderWidth: 1.5, borderRadius: 3, borderColor: "#dbdbdb" }}>
    {pagamentosVisiveis.map((p) => {
const cobranca = cobrancas.find(c => c.id === p.cobrancaId);
...
>
  ...
  <View style={styles.modalOverlay}>
    <View style={styles.modalCard}>
      ...
      <Text style={{ fontWeight: '700', marginBottom: 10, fontSize: 16 }}>Nova cobrança</Text>
  ...
  ...prev,
  {
    ...
    data: new Date(consultaSelecionada.data).toLocaleDateString(),
...
<Modal visible={modalPagamento} transparent animationType="fade">
  <View style={styles.modalOverlay}>
    <View style={styles.modalCard}>
      ...
      <Text style={{ fontWeight: '700', fontSize: 16, marginBottom: 10 }}>Registrar pagamento</Text>

      <Text style={{marginBottom: 5}}>Selecione o dia em que foi pago:</Text>
      <TouchableOpacity onPress={() => setMostrarDatePicker(true)}>
  <Text style={{ marginTop: 5, borderWidth: 4, padding: 7, borderColor: "#a9a9a9", borderRadius: 10, width: "40%" }}>
...
{mostrarDatePicker && (
  ...
  />
      ...
      {['pix', 'credito', 'debito', 'dinheiro'].map((f) => (
        <TouchableOpacity key={f} onPress={() => setFormaPagamento(f as any)}>
  ...
  <>
    <Text style={{ marginTop: 10 }}>Parcelar em quantas vezes?</Text>
  ...
  const qtdParcelas = formaPagamento === 'credito' ? Number(parcelas) : 1;
...
<Modal visible={modalParcela} transparent animationType="fade">
  ...
  setCobrancas(prevCobrancas =>
    prevCobrancas.map(cb => {
      if (cb.id !== cobrancaParcela.id || !cb.parcelas) return cb;
...
>
  <View style={styles.modalContainer}>
    ...
    >
      <Text style={{ color: '#fff', fontSize: 16, fontWeight: '600' }}>Fechar</Text>
    </TouchableOpacity>
...
>
  ...
  {arquivoSelecionado && getIconByFileName(arquivoSelecionado.name)}
</View>
