...
type PacienteContextType = {
  pacientes: Paciente[];
  adicionarPaciente: ( p: Omit<Paciente, 'id' | 'historico' | 'odontograma' | 'planejamentoOrto'>) => void;
  adicionarHistorico: (
    pacienteNome: string,
...
export function PacienteProvider({ children }: { children: ReactNode }) {
 ...
 async function carregarPacientes() {
  ...
  await updateDoc(
    doc(db, "usuarios", user.uid, "pacientes", id),
    dados
  );
...
async function adicionarImagem(pacienteId: string, uri: string) {
  ...
  await updateDoc(
    doc(db, "usuarios", user.uid, "pacientes", pacienteId),
...
async function removerImagem(pacienteId: string, uri: string) {
  ...
  const novasImagens =
    paciente.imagens?.filter(img => img !== uri) || [];
...
async function removerArquivo(pacienteId: string, uri: string) {
  ...
  const novosArquivos =
    paciente.arquivos?.filter(a => a.uri !== uri) || [];
 ...
 async function adicionarHistorico(
  ...
  await updateDoc(
    doc(db, "usuarios", user.uid, "pacientes", paciente.id),
 ...
 async function salvarDente(pacienteId: string, dente: DenteRegistro) {
  ...
  const existe = paciente.odontograma.find(
    (d) => d.numero === dente.numero
  );
...
async function salvarPlanejamentoOrto(
  ...
  setPacientes((prev) =>
    prev.map((p) =>
      p.id === pacienteId
        ? { ...p, planejamentoOrto: planejamento }
        : p
    )
  );
}
