...
import { agendarNotificacaoConsulta, cancelarNotificacaoConsulta } from "../services/notificacoes"
...
useEffect(() => {
 ...
 async function adicionarConsulta(novaConsulta: Consulta) {
  ...
  await deleteDoc(
    doc(db, "usuarios", user.uid, "consultas", id)
  );
...
async function enviarConfirmacaoWhatsApp(consultaId: string) {
  ...
  const mensagem = 
`Olá ${consulta.paciente}, tudo bem?
...
Poderia confirmar, por favor?`;
  ...
  // SALVA NO FIRESTORE
  await updateDoc(
    doc(db, "usuarios", user.uid, "consultas", consultaId),
    {
  ...
  setConsultas(prev =>
    prev.map(c =>
      c.id === consultaId
        ? { ...c, confirmacaoWhatsapp: "pendente" }
        : c
    )
  );
}
...
async function marcarComoConfirmada(id: string) {
  ...
  setConsultas(prev =>
    prev.map(c =>
      c.id === id
        ? { ...c, confirmacaoWhatsapp: "confirmado" }
        : c
    )
  );
}
